...
public class BasketsFiller  extends BaseFiller {
    ...
    protected String insert_sales (Map<Integer, Region>  regionsMap, boolean forSales) {        
        ...
        if (forSales) {
            // тут иммитируем продажу с "тяжелыми запросами". 
            // Это замер на точку графика - количество вставок нужно ограничить            
            countLimiter = BaseFiller.getMEASURE_INSERT_LIMIT ();
        }
    ...
    String hard = """
                  SELECT SUM(price_of_one_piece) FROM sales_09_mln WHERE order_number IN 
                  (SELECT order_numbers_dictionary_09_mln.order_number FROM
                  order_numbers_dictionary_09_mln WHERE 
                  order_date>'2023-01-01' AND order_date < '2023-01-09'
                  AND order_numbers_dictionary_09_mln.basket_price > 10000
                  AND order_numbers_dictionary_09_mln.basket_price < 40000)
                  AND (sales_09_mln.product_id < 7000
                  AND sales_09_mln.product_id > 6000)
                  OR (sales_09_mln.product_id < 3000
                  AND sales_09_mln.product_id > 2000
                  );""";
    protected void doHard (ProductBasket basket) throws SQLException
    {
        try(PreparedStatement preparedStatement = conn.prepareStatement(hard);) {
                ResultSet resultSet = preparedStatement.executeQuery();        
        }
        
        
        //toDo вставить тяжелый запрос
        //logback.info(String.format(" >>>>>>>>>>>>>>>>>>>>>>>>>>>>> вошли в doHard() БД %s", dbName));
        int i = 0;
    }
        ...
        do { // цикл повторной вставки, если с первого раза вставить не удалось. Если удалось - будет return из цикла for 
            ...
            for (int i = 1; i <= size; i++) {
                ...
                try (PreparedStatement preparedStatement = conn.prepareStatement(sqlOper);) {
                    ...
                    logback.warn(String.format("%s *******   ошибка вставки(SQLException) в таблицу %s - будет повторная попытка вставки. Исключение: %s", ConfJDBC.LOG_PEFIX_ERR, saleTableName, exc.getMessage()));
                    break;
                }                
            } //  цикл закончился
    ...
    int table_number = 3000001;

    public static final String INSERT_BASKET_KEY = "INSERT INTO %s (order_number, region_id, market_id, terminal_id, terminal_basket_number, basket_price, order_time, order_date, comment) VALUES (?,?, ?, ?, ?, ?, ?, ?, ?) RETURNING order_number;";
    protected Integer getReturningOrder_number(ProductBasket productBasket) throws Exception {
        ...
        try(PreparedStatement preparedStatement = conn.prepareStatement(sqlOper);) {
            ...
            preparedStatement.setInt      (1, table_number);
            preparedStatement.setInt      (2, productBasket.getRegion_id());
            preparedStatement.setInt      (3, productBasket.getMarket_id());
            preparedStatement.setInt      (4, productBasket.getTerminal_id());
            preparedStatement.setInt      (5, productBasket.getTerminal_basket_number());
            preparedStatement.setInt      (6, productBasket.getBasket_price());
            preparedStatement.setTimestamp(7, productBasket.getSaleTimestamp());
            preparedStatement.setDate     (8, productBasket.getSaleDate());
            preparedStatement.setString   (9, "запись о покупке");
